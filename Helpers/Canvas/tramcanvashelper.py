...
from Provenance.plot_provenance import (
    create_temporary_provenance_image
)


def create_1D_tram_canvas(
    ...
    """
    Create the 1D tram histogram canvas.
    ...
    if show_provenance:
        ...
        (
            provenance_image,
            provenance_path,
        ) = _prepare_provenance_image(
            histograms=unique_histograms,
            target_width=provenance_pixel_width,
            target_height=provenance_pixel_height,
        )
...
def _prepare_provenance_image(
    histograms: list[Any],
    target_width: int,
    target_height: int,
):
    """
    Create and open a provenance image that has already been
    rendered for the exact ROOT provenance-pad dimensions.
    ...
    if provenance_image is None:
        raise RuntimeError(
            "Could not open provenance image: "
            f"{provenance_path}"
        )
