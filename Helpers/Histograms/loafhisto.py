...
from Histograms.validateloafhisto import validate_create_loaf_histograms_arguments
from SaveFiles.savehistograms import save_histograms
from Provenance.provenance import register_provenance

def create_loaf_histograms(
    ...
    quantity_specs = _get_loaf_quantities(
        quantities
    )
    ...
    _validate_dataframe_columns(
        ...
        mode_name="loaf histogram creation",
    )

    histogram_config = _load_loaf_histogram_defaults(
    ...
    if saveHistograms:
        save_histograms(
            ...
            histogram_data= histogram_data_1d + histogram_data_2d,
        )

    return _index_loaf_histograms(
        histogram_data_1d=(
...
def _book_histogram_actions(
    dataframes: list[Any],
    dataframe_labels: list[str],
    one_dimensional_specs: list[LoafQuantitySpec],
    two_dimensional_specs: list[LoafQuantitySpec],
...
def _collect_1d_histograms(
    ...
    for job in jobs:
        ...
        register_provenance(
            histogram,
            kind="histogram",
            operation="LOAF histogram 1D",
            parameters={
                "quantity": job["quantity"],
                "dataframe_label": job["dataset_label"],
                "bins": job["bins"],
                "range": [
                    job["minimum"],
                    job["maximum"],
                ],
                "dataframe_entries": entries,
                "histogram_entries": int(
...
def _prepare_2d_settings(
    all_quantity_specs: list[LoafQuantitySpec],
    two_dimensional_specs: list[LoafQuantitySpec],
    bins,
    ranges,
    config: dict[str, Any],
    ...
    bin_settings = _get_loaf_2d_bins(
        quantity_specs=all_quantity_specs,
        bins=bins,
    )

    range_settings = _get_loaf_2d_ranges(
        quantity_specs=all_quantity_specs,
        ranges=ranges,
    )
    ...
    for spec in two_dimensional_specs:
        ...
        print(
            f"loaf histogram 2D: {spec.expression}, "
...
def _prepare_1d_settings(
    ...
    bin_counts, histogram_ranges = (
        _resolve_loaf_histogram_settings(
            quantities=quantities,
            bins=bins,
            ranges=ranges,
            config=config,
...
def _get_loaf_quantities(
    ...
    "Itime:Iedeposit"
        Two-dimensional histogram using ROOT's X:Y convention.
...
def _load_loaf_histogram_defaults(
...
def _resolve_loaf_histogram_settings(
    quantities: list[str],
    ...
    """
    Resolve histogram settings for loaf quantities.
    ...
    4. JSON loaf default entry
    5. Built-in fallback

    Built-in fallbacks
    ------------------
    Ordinary loaf quantity:
    ...
    loaf_config = config.get("loaf", {})

    if not isinstance(loaf_config, Mapping):
        raise HistogramError(
            "'loaf' in the histogram configuration "
            "must be an object."
        )

    default_entry = loaf_config.get("default")
    quantity_config = loaf_config.get("quantities", {})
    prefix_config = loaf_config.get("prefixes", {})

    if not isinstance(quantity_config, Mapping):
        raise HistogramError(
            "'loaf.quantities' must be an object."
        )

    if not isinstance(prefix_config, Mapping):
        raise HistogramError(
            "'loaf.prefixes' must be an object."
        )

    default_bins, default_range = (
        _parse_histogram_default_entry(
            default_entry,
            context="loaf default",
        )
    )
    ...
    for index, (
        ...
        exact_bins, exact_range = (
            _parse_histogram_default_entry(
                ...
                context=f"loaf quantity {quantity!r}",
            )
        )
        ...
        prefix_bins, prefix_range = (
            _parse_histogram_default_entry(
                prefix_entry,
                context=f"loaf prefix for {quantity!r}",
            )
        )
        ...
        if (
            ...
            raise HistogramError(
                f"The bin count for loaf quantity "
        ...
        if emit_log:
            print(
                f"loaf histogram {index}: {quantity}, "
...
def _make_loaf_histogram_name(
    quantity: str,
    dataset_index: int,
) -> str:
    return (
        f"h_loaf_{_make_safe_root_name(quantity)}_"
...
def _resolve_loaf_column_defaults(
    column: str,
    config: dict[str, Any],
) -> tuple[
    int | None,
    ...
    """
    Resolve default bins and range for one loaf column.

    Priority
    --------
    1. Exact quantity configuration
    2. Prefix configuration
    3. General loaf default
    4. Built-in fallback
    """
    loaf_config = config.get("loaf", {})
    ...
    quantity_config = loaf_config.get(
        "quantities",
        {},
    )
    prefix_config = loaf_config.get(
        "prefixes",
        {},
    )
    ...
    exact_bins, exact_range = (
        _parse_histogram_default_entry(
            ...
            context=f"loaf quantity {column!r}",
        )
    )
    ...
    prefix_bins, prefix_range = (
        _parse_histogram_default_entry(
            prefix_entry,
            context=f"loaf prefix for {column!r}",
        )
    )
...
def _make_loaf_2d_histogram_name(
    spec: LoafQuantitySpec,
    dataset_index: int,
) -> str:
    return (
    ...
    for item in all_histogram_data:
        ...
        if dataframe_label in quantity_histograms:
            raise HistogramError(
                ...
                "Loaf histograms. Duplicate label "
