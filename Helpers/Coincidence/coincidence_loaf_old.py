...
def coincidence_loaf_old(
    ...
    ****** Deprecated with the new loaf format ***********
    ...
    # Second pass
    #
    # The old implementation looked up:
    #
    #   (run, subrun, event) -> Result
    #
    # for every row and then compared rdfentry_.
    #
    # The optimized helper directly looks up:
    #
    #   rdfentry_ -> double
    #
    # --------------------------------------------------------------
    ...
    for item in contexts:
        ...
        register_provenance(
            output_dataframe,
            kind="dataframe",
            operation="old LOAF coincidence",
            parameters={
                "detectors": list(
                    normalized_detectors
                ),
                ...
                "subrun_column": subrun_column,
                "time_column": time_column,
                "output_column": "time_coinc",
                "time_definition": (
                    "average of detectors[0] and detectors[1]"
                ),
                "written_on_detector": (
                    normalized_detectors[0]
                ),
                "requires_all_detectors": True,
                "duplicate_detector_policy": (
                    "smallest rdfentry_"
                ),
                "deprecated": True,
            },
