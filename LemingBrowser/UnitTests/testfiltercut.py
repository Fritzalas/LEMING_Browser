"""
200 standalone tests for global_filter_cut_rdataframe.
...
# ===========================================================================
# Empty-vector preservation regression cases
# ===========================================================================
#
# These cases exercise the intended ApplyMask behavior:
#
#   - if an unrelated vector is empty for a surviving row, keep it empty;
#   - aligned non-empty vectors are masked normally;
#   - scalar columns remain untouched while the row survives;
#   - rows with an empty mask are still rejected by Any(mask).
#
# A non-empty vector whose size differs from the mask is intentionally NOT
# covered here as a success case: that must still raise.
# ===========================================================================


EMPTY_VECTOR_CASES = [
    (
        "empty_unrelated_numeric_vector",
        [
            {
                "Energy": [1, 2, 3],
                "Time": [10, 20, 30],
    ...
    (
        "abs_difference_with_empty_right_collection",
        [
            {
                "Z1L": [10.0, 20.0, 30.0, 40.0],
                "Z2L": [10.1, 21.0, 29.9, 45.0],
                "Y1L": [1, 2, 3, 4],
                "Z1R": [],
                "Z2R": [],
                "Event": 101,
    ...
    (
        "alternating_left_and_right_collections",
        [
            {
                "Z1L": [10.0, 20.0],
                "Z2L": [10.1, 25.0],
    ...
    (
        "empty_string_vector_survives_numeric_mask",
        [
            {
                "Energy": [10, 50, 100],
                "Time": [1, 2, 3],
    ...
    (
        "empty_bool_vector_survives_compound_mask",
        [
            {
                "Energy": [20, 60, 100, 180],
        ...
        "EventGood && Good && Energy >= 50 && Energy < 150",
        [
            {
                "Energy": [60, 100],
                "Good": [True, True],
    ...
    (
        "multiple_empty_unrelated_collections",
        [
            {
                "X": [1, 2, 3, 4, 5],
                "Y": [10, 20, 30, 40, 50],
            ...
            },
            {
                ...
                "RightFloat": [1.5],
                "RightBool": [True],
                "RightName": ["type-anchor"],
                "Threshold": 25,
            },
        ],
        "Good && Y >= Threshold && X < 5",
        [
            {
                "X": [3, 4],
                "Y": [30, 40],
    ...
    (
        "cut_list_with_empty_unrelated_vectors",
        [
            {
                "Particle": ["muon", "proton", "muon", "electron"],
                "Energy": [70, 300, 120, 500],
    ...
    (
        "deep_nested_mask_with_empty_opposite_side",
        [
            {
                "Z1L": [10.0, 20.0, 30.0, 40.0, 50.0],
                "Z2L": [10.05, 19.0, 30.1, 44.0, 49.9],
                "Energy": [100, 200, 300, 400, 500],
                "Good": [True, True, False, True, True],
                "Particle": ["muon", "muon", "proton", "muon", "proton"],
                "Z1R": [],
                "Z2R": [],
                "RunType": "Physics",
                "Force": False,
        ...
        (
            'RunType == "Physics" && ('
            '(Particle == "muon" && Good && Energy >= 100) || '
            '(Particle == "proton" && Energy >= 450) || '
            "Force"
            ") && abs(Z1L - Z2L) < 0.2"
        ),
        [
    ...
    (
        "surviving_rows_keep_different_empty_columns",
        [
            {
                "A": [1, 2, 3],
                "B": [10, 20, 30],
    ...
    (
        "scalar_force_plus_vector_mask_with_empty_collection",
        [
            {
                "Force": False,
                "Energy": [10, 60, 100, 20],
...
# ===========================================================================
# Empty logical-mask operand regression cases
# ===========================================================================
#
# These 40 cases test the special logical semantics:
#
#   nonempty || []  -> nonempty
#   [] || nonempty  -> nonempty
#   nonempty && []  -> all-false mask with nonempty length
#   [] && nonempty  -> all-false mask with nonempty length
#   [] || []        -> []
#   [] && []        -> []
#
# Genuine NON-EMPTY size mismatches are still errors and are covered by the
# existing mismatch tests elsewhere in this suite.
# ===========================================================================


EMPTY_LOGICAL_OPERAND_CASES = [
    (
        "or_nonempty_left_empty_right",
        [{"A": [0, 2, 3], "B": [], "Payload": [10, 20, 30]}],
        "(A > 1) || (B > 0)",
        [{"A": [2, 3], "B": [], "Payload": [20, 30]}],
    ),
    (
        "or_empty_left_nonempty_right",
        [{"A": [], "B": [0, 2, 3], "Payload": [10, 20, 30]}],
        "(A > 0) || (B > 1)",
        [{"A": [], "B": [2, 3], "Payload": [20, 30]}],
    ),
    (
        "and_nonempty_left_empty_right_rejects",
        [{"A": [1, 2, 3], "B": [], "Payload": [10, 20, 30]}],
        "(A > 0) && (B > 0)",
        [],
    ),
    (
        "and_empty_left_nonempty_right_rejects",
        [{"A": [], "B": [1, 2, 3], "Payload": [10, 20, 30]}],
        "(A > 0) && (B > 0)",
        [],
    ),
    (
        "or_both_empty_rejects",
        [{"A": [], "B": [], "Scalar": 7}],
        "(A > 0) || (B > 0)",
        [],
    ),
    (
        "and_both_empty_rejects",
        [{"A": [], "B": [], "Scalar": 7}],
        "(A > 0) && (B > 0)",
        [],
    ),
    (
        "or_all_false_nonempty_with_empty_rejects",
        [{"A": [1, 2, 3], "B": [], "Payload": [4, 5, 6]}],
        "(A > 100) || (B > 0)",
        [],
    ),
    (
        "or_mixed_nonempty_with_empty_filters_payload",
        [{"A": [1, 20, 3, 40], "B": [], "Payload": [10, 20, 30, 40]}],
        "(A >= 20) || (B > 0)",
        [{"A": [20, 40], "B": [], "Payload": [20, 40]}],
    ),
    (
        "or_empty_with_mixed_nonempty_filters_payload",
        [{"A": [], "B": [1, 20, 3, 40], "Payload": [10, 20, 30, 40]}],
        "(A > 0) || (B >= 20)",
        [{"A": [], "B": [20, 40], "Payload": [20, 40]}],
    ),
    (
        "and_all_true_nonempty_with_empty_still_rejects",
        [{"A": [10, 20, 30], "B": [], "Payload": [1, 2, 3]}],
        "(A > 0) && (B > 0)",
        [],
    ),

    (
        "chain_or_empty_middle",
        [{"A": [0, 2, 0], "B": [], "C": [0, 0, 3], "Payload": [10, 20, 30]}],
        "(A > 1) || (B > 0) || (C > 2)",
        [{"A": [2, 0], "B": [], "C": [0, 3], "Payload": [20, 30]}],
    ),
    (
        "chain_and_empty_then_or_recovers_from_c",
        [{"A": [1, 2, 3], "B": [], "C": [0, 5, 0], "Payload": [10, 20, 30]}],
        "(A > 0) && (B > 0) || (C > 1)",
        [{"A": [2], "B": [], "C": [5], "Payload": [20]}],
    ),
    (
        "precedence_or_with_empty_and_branch",
        [{"A": [0, 2, 0], "B": [], "C": [1, 1, 1], "Payload": [10, 20, 30]}],
        "(A > 1) || (B > 0) && (C > 0)",
        [{"A": [2], "B": [], "C": [1], "Payload": [20]}],
    ),
    (
        "empty_or_compound_nonempty_and",
        [{"A": [], "B": [1, 2, 3], "C": [3, 0, 3], "Payload": [10, 20, 30]}],
        "(A > 0) || ((B > 1) && (C > 1))",
        [{"A": [], "B": [3], "C": [3], "Payload": [30]}],
    ),
    (
        "empty_and_nonempty_then_or_c",
        [{"A": [], "B": [1, 2, 3], "C": [0, 4, 0], "Payload": [10, 20, 30]}],
        "(A > 0) && (B > 0) || (C > 1)",
        [{"A": [], "B": [2], "C": [4], "Payload": [20]}],
    ),
    (
        "parenthesized_or_empty_then_and_c",
        [{"A": [0, 2, 3], "B": [], "C": [1, 0, 1], "Payload": [10, 20, 30]}],
        "((A > 1) || (B > 0)) && (C > 0)",
        [{"A": [3], "B": [], "C": [1], "Payload": [30]}],
    ),
    (
        "parenthesized_empty_or_b_then_and_c",
        [{"A": [], "B": [0, 2, 3], "C": [1, 0, 1], "Payload": [10, 20, 30]}],
        "((A > 0) || (B > 1)) && (C > 0)",
        [{"A": [], "B": [3], "C": [1], "Payload": [30]}],
    ),
    (
        "parenthesized_and_empty_then_or_c",
        [{"A": [1, 2, 3], "B": [], "C": [5, 0, 6], "Payload": [10, 20, 30]}],
        "((A > 0) && (B > 0)) || (C > 1)",
        [{"A": [1, 3], "B": [], "C": [5, 6], "Payload": [10, 30]}],
    ),
    (
        "or_nonempty_with_parenthesized_empty_and_c",
        [{"A": [0, 2, 0], "B": [], "C": [1, 1, 1], "Payload": [10, 20, 30]}],
        "(A > 1) || ((B > 0) && (C > 0))",
        [{"A": [2], "B": [], "C": [1], "Payload": [20]}],
    ),
    (
        "empty_and_b_or_compound_c_d",
        [{"A": [], "B": [1, 2, 3], "C": [0, 5, 6], "D": [1, 1, 0]}],
        "((A > 0) && (B > 0)) || ((C > 1) && (D > 0))",
        [{"A": [], "B": [2], "C": [5], "D": [1]}],
    ),
    ...
    (
        "bool_or_empty_bool",
        [{"P": [True, False, True], "Q": [], "X": [1, 2, 3]}],
        "P || Q",
        [{"P": [True, True], "Q": [], "X": [1, 3]}],
    ),
    (
        "empty_bool_or_bool",
        [{"P": [], "Q": [False, True, False], "X": [1, 2, 3]}],
        "P || Q",
        [{"P": [], "Q": [True], "X": [2]}],
    ),
    (
        "bool_and_empty_bool_rejects",
        [{"P": [True, True, True], "Q": [], "X": [1, 2, 3]}],
        "P && Q",
        [],
    ),
    (
        "negated_bool_or_empty_bool",
        [{"P": [True, False, True], "Q": [], "X": [1, 2, 3]}],
        "(!P) || Q",
        [{"P": [False], "Q": [], "X": [2]}],
    ),
    (
        "bool_or_negated_empty_bool",
        [{"P": [True, False, True], "Q": [], "X": [1, 2, 3]}],
        "P || (!Q)",
        [{"P": [True, True], "Q": [], "X": [1, 3]}],
    ),
    ...
    (
        "string_vector_or_empty_string_vector",
        [
            {"Name": ["muon", "electron", "muon"], "OtherName": [], "E": [10, 20, 30]},
            {"Name": ["anchor"], "OtherName": ["anchor"], "E": [0]},
        ],
        '(Name == "muon") || (OtherName == "target")',
        [
            {"Name": ["muon", "muon"], "OtherName": [], "E": [10, 30]},
        ],
    ),
    (
        "empty_string_vector_or_string_vector",
        [
            {"Name": [], "OtherName": ["x", "target", "y"], "E": [10, 20, 30]},
            {"Name": ["anchor"], "OtherName": ["anchor"], "E": [0]},
        ],
        '(Name == "muon") || (OtherName == "target")',
        [
            {"Name": [], "OtherName": ["target"], "E": [20]},
        ],
    ),
    (
        "string_vector_and_empty_string_vector_rejects",
        [
            {"Name": ["muon", "muon"], "OtherName": [], "E": [10, 20]},
            {"Name": ["anchor"], "OtherName": ["anchor"], "E": [0]},
        ],
        '(Name == "muon") && (OtherName == "target")',
        [],
    ...
    (
        "arithmetic_mask_or_empty",
        [{"A": [1, 2, 3], "B": [3, 3, 3], "Empty": [], "Payload": [10, 20, 30]}],
        "(A + B >= 5) || (Empty > 0)",
        [{"A": [2, 3], "B": [3, 3], "Empty": [], "Payload": [20, 30]}],
    ),
    (
        "empty_or_arithmetic_mask",
        [{"Empty": [], "A": [1, 2, 3], "B": [3, 3, 3], "Payload": [10, 20, 30]}],
        "(Empty > 0) || (A + B >= 5)",
        [{"Empty": [], "A": [2, 3], "B": [3, 3], "Payload": [20, 30]}],
    ),

    (
        "compound_left_mask_or_empty",
        [{"A": [1, 2, 3], "B": [4, 6, 2], "Empty": [], "Payload": [10, 20, 30]}],
        "((A > 1) && (B < 5)) || (Empty > 0)",
        [{"A": [3], "B": [2], "Empty": [], "Payload": [30]}],
    ),
    (
        "empty_or_compound_right_mask",
        [{"Empty": [], "A": [1, 2, 3], "B": [4, 6, 2], "Payload": [10, 20, 30]}],
        "(Empty > 0) || ((A > 1) && (B < 5))",
        [{"Empty": [], "A": [3], "B": [2], "Payload": [30]}],
    ),
    (
        "or_empty_then_and_third_mask",
        [{"A": [0, 2, 3], "Empty": [], "C": [1, 1, 0], "Payload": [10, 20, 30]}],
        "((A > 1) || (Empty > 0)) && (C > 0)",
        [{"A": [2], "Empty": [], "C": [1], "Payload": [20]}],
    ),
    (
        "empty_or_a_then_and_third_mask",
        [{"Empty": [], "A": [0, 2, 3], "C": [1, 1, 0], "Payload": [10, 20, 30]}],
        "((Empty > 0) || (A > 1)) && (C > 0)",
        [{"Empty": [], "A": [2], "C": [1], "Payload": [20]}],
    ),
    (
        "and_empty_then_or_third_mask",
        [{"A": [1, 2, 3], "Empty": [], "C": [0, 5, 0], "Payload": [10, 20, 30]}],
        "((A > 0) && (Empty > 0)) || (C > 1)",
        [{"A": [2], "Empty": [], "C": [5], "Payload": [20]}],
    ),

    (
        "scalar_true_gate_vector_or_empty",
        [{"Enabled": True, "A": [0, 2, 3], "Empty": [], "Payload": [10, 20, 30]}],
        "(Enabled && A > 1) || (Empty > 0)",
        [{"Enabled": True, "A": [2, 3], "Empty": [], "Payload": [20, 30]}],
    ),
    (
        "scalar_false_gate_empty_or_vector",
        [{"Enabled": False, "Empty": [], "A": [0, 2, 3], "Payload": [10, 20, 30]}],
        "(Enabled && Empty > 0) || (A > 1)",
        [{"Enabled": False, "Empty": [], "A": [2, 3], "Payload": [20, 30]}],
    ),
    (
        "two_rows_opposite_empty_sides",
        [
            {"A": [0, 2, 3], "B": [], "Payload": [10, 20, 30], "Row": 1},
            {"A": [], "B": [4, 0, 5], "Payload": [40, 50, 60], "Row": 2},
        ],
        "(A > 1) || (B > 1)",
        [
            {"A": [2, 3], "B": [], "Payload": [20, 30], "Row": 1},
            {"A": [], "B": [4, 5], "Payload": [40, 60], "Row": 2},
        ],
    ),
    (
        "operand_empty_plus_unrelated_empty_columns",
        [
            {
                "A": [0, 2, 3],
                "OperandEmpty": [],
    ...
    (
        "long_or_chain_with_two_empty_operands",
        [
            {
                "A": [0, 2, 0, 4],
                "E1": [],
...
def collect_tests():
    ...
    # 10 empty-vector preservation regression tests.
    tests.extend(_case_tests("empty_vector", EMPTY_VECTOR_CASES))

    # 40 empty logical-mask operand regression tests.
    tests.extend(
        _case_tests(
            "empty_logical_operand",
            EMPTY_LOGICAL_OPERAND_CASES,
        )
    )

    assert len(tests) == 200, (
        f"Test suite must contain exactly 200 tests, got {len(tests)}."
    )
